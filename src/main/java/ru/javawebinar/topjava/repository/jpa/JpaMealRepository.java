...
public class JpaMealRepository implements MealRepository {
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        Query query = em.createQuery("SELECT m FROM Meal m WHERE m.dateTime>=:startDate AND m.dateTime<=:endDate AND m.user=:userId");
        return query.setParameter("userId", userId).setParameter("startDate", startDate).getResultList();
    }
}